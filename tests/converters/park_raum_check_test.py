...
class ParkRaumCheckSachsenheimOnStreetPushConverterTest:
    @staticmethod
    def test_get_static_parking_sites(mocked_config_helper: Mock, request_helper: RequestHelper):
        with get_data_path('park_raum_check_sachsenheim_on_street.geojson').open() as sachsenheim_file:
            sachsenheim_data = json.loads(sachsenheim_file.read())

        converter = ParkRaumCheckSachsenheimOnStreetPushConverter(
...
class SachsenheimOffStreetPushConverterTest:
    @staticmethod
    def test_get_static_parking_sites(mocked_config_helper: Mock, request_helper: RequestHelper):
        with get_data_path('park_raum_check_sachsenheim_off_street.geojson').open() as sachsenheim_file:
            sachsenheim_data = json.loads(sachsenheim_file.read())

        converter = ParkRaumCheckSachsenheimOffStreetPushConverter(
            config_helper=mocked_config_helper,
            request_helper=request_helper,
        )
