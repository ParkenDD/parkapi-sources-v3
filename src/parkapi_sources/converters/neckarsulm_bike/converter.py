    ...
    def handle_csv(
        ...
        for row in data[1:]:
            ...
            if input_dict['name'] and input_dict['additional_name']:
                input_dict['name'] = (
                    f'{input_dict["name"]}, {input_dict["additional_name"]}'
                )
            ...
            if input_dict['lat'] and input_dict['lon']:
                coordinates = self.proj(
                    float(input_dict['lon']), float(input_dict['lat']), inverse=True
                )
            ...
            try:
                static_parking_site_inputs.append(
                    self.static_parking_site_validator.validate(input_dict)
                )
