...
class NagoldBikePushConverter(JsonConverter, ParkingSiteBaseConverter):
    ...
    def handle_json(self, data: dict | list) -> tuple[list[StaticParkingSiteInput], list[ImportParkingSiteException]]:
        ...
        for parking_site_dict in parking_sites_input.features:
            properties: AnyDict = parking_site_dict.get('properties', {})
            if self._is_placeholder(properties) or self._is_private(properties):
                continue
            ...
            try:
                ...
                uid: Any = properties.get('OBJECTID')
    ...
    def _is_private(properties: AnyDict) -> bool:
        """
        Privately operated installations are not public bike parking and therefore not integrated.
        """
        return str(properties.get('Betreiber') or '').strip().lower() == 'privat'
